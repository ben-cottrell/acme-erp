...
- Accessible supervisor review screens.
...
|---|---|---|---|
| Fulfilment Supervisor | Fulfilment control user. | Monitor queues, approve/reject exceptions, review partial fulfilment and reversals. | Exception queues, decision context, and filters. |

## 6. User Journeys and Workflows

- **Monitor workload**: supervisor opens the workload view and reviews released, in-progress, blocked, and aging fulfilment work.
...
|---|---|---|---|---|
| FSU-APP-001 | Workload queue | The application shall display fulfilment workload and exception queues from Order Fulfilment. | Must | Given authorized supervisor access, when the workload view loads, then queues show status, age, exception type, SKU, operator, and task/order references. |
...
| FSU-APP-004 | Self-approval prevention | The application shall prevent or surface self-approval denials when Order Fulfilment indicates a self-approval conflict. | Must | Given the supervisor created/requested the exception, when approval is attempted, then the action is denied and the UI explains the conflict. |
| FSU-APP-005 | Filtering | The application shall support filtering by exception type, SKU, operator, date, courier, order, status, age, and channel. | Should | Given filters are applied, when results load, then paginated domain results are shown. |

## 8. UI, Accessibility, and Usability Requirements

- Workload and exception queues shall prioritize exception age, business impact, and blocked tasks without hiding lower-priority queues.
...
|---|---|---|---|---|
| FSU-INT-001 | Load workload and exception queues | Order Fulfilment | Filters and queue/status data. | Show the returned workload and business exception states. |
...
## 11. Workload and Exception Queue Requirements

| View | Audience | Purpose | Filters |
|---|---|---|---|
| Fulfilment Workload | Fulfilment Supervisor | Monitor work volume and aging. | Status, age, SKU, channel, operator. |
| Exception Queue | Fulfilment Supervisor | Resolve blocked tasks. | Exception type, courier, SKU, operator, date, order. |
| Partial Fulfilment Review | Fulfilment Supervisor, Sales | Track backorder decisions. | Order, customer, SKU, date, status. |
| Shipment References | Fulfilment Supervisor | Review accepted shipment, tracking, and label references. | Courier, date, task, status. |
...
- Workload and exception queues shall use paginated queries and stable filters/sorting.
...
- Partial fulfilment approval is allowed for short pick or unavailable inventory scenarios returned by Order Fulfilment and does not require customer confirmation in MVP.

## 16. Acceptance Summary

The Fulfilment Supervisor requirements are complete for MVP when they define workload monitoring, exception review, controlled approvals, cross-domain context, security, and explicit MVP defaults without assigning durable fulfilment, sales, or inventory state to the application.

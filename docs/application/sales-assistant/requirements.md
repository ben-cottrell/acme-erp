...
| SA-APP-008 | History | The application shall display sales order status history, amendment history, approval history, and release exception history available from Sales. | Should | Given a user has permission, when history is requested, then entries show actor, date/time, action, status, and reason where supplied. |

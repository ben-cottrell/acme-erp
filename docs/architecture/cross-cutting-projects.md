...
This document defines when shared projects are allowed and which cross-cutting concerns should be centralized across modules.

## Core Rule

Create shared projects only for concerns that affect multiple APIs or services and have stable, module-neutral behavior. Do not create shared projects for module-specific business rules.
...
Project names may be adjusted during implementation, but the boundary rule remains: shared code is cross-cutting, stable, and not owned by one business module.
...
- Module-owned EF Core entities or `DbContext` types.
- Module-specific API request/response contracts unless they are explicit published integration contracts.
- Product, customer, supplier, or courier business rules owned by one module.
...
- External record IDs and owning module.
- Schema version.
- Timestamp.
...
Security shared code may provide policy primitives and claim mapping helpers, but module APIs remain responsible for final authorization decisions.
...
Module APIs own the business meaning of audit events. For example, `PurchaseOrderApproved`, `InventoryAdjustmentPosted`, and `FulfilmentCompleted` are module-owned events even when they use a shared audit envelope.
...
- Shared projects should remain small enough that module teams can understand their dependency impact.

## Review Checklist

- [ ] Shared code is used by multiple services or tests.
- [ ] Module-specific domain behavior remains inside the owning module.
- [ ] Shared security code does not bypass module authorization decisions.
- [ ] Shared audit code standardizes shape without owning module semantics.
- [ ] Integration contracts clearly name external IDs and owning modules.

...
| Security and Audit and Security Administration are de-scoped for MVP. | The MVP focuses on business workflows while Authentik and Gravitee provide identity and ingress. | Do not generate a Security and Audit bounded context, Security Administration application, or active compliance workflows. Access permissions are documented in the owning application/domain requirements. |
...
| Order Fulfilment | Partial fulfilment is allowed and sends remaining quantity to Sales as backordered. |
